...
| PATCH  | /users/{userId}         | 유저 정보 수정    | userId (path)     | { "name": string "password" : string }                  | { "id": long, "name": string, "email": string, "createdAt": DATETIME, "modifiedAt": DATETIME }                                    | 200 OK         | 400 Bad Request <br> 404 Not Found |
